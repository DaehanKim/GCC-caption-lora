...
class T5DenseActDense(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if hasattr(self.wo, "weight"):
            hidden_states = hidden_states.to(self.wo.weight.dtype)
        else: # then lora layer
            hidden_states = hidden_states.to(self.wo.base_layer.weight.dtype)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            lm_logits = lm_logits.to(torch.float32) # cast to float32 for full precision loss
